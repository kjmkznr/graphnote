...
export function getCompletions(
  ...
  if (type === 'nodeType') {
    ...
    candidates = context.edgeTypes.map((t) => ({ label: t, kind: 'edgeType' as const }));
  } else if (type === 'property') {
    candidates = context.propertyKeys.map((k) => ({ label: k, kind: 'property' as const }));
  } else {
    candidates = CYPHER_KEYWORDS.map((k) => ({ label: k, kind: 'keyword' as const }));
  }
