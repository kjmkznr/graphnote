...
import { applyCompletion, getCompletions } from './cypherAutocomplete.js';
import { DOM_IDS } from './domIds.js';
import { byId, escHtml } from './domUtils.js';
...
export class QueryPanel {
  ...
  constructor() {
    ...
    this.elBookmarkDeleteBtn.addEventListener('click', () => {
      ...
      if (!selectedOpt?.dataset.id) return;
  ...
  private selectItem(idx: number): void {
    ...
    if (!item) return;
    const cursorPos = this.elInput.selectionStart ?? this.elInput.value.length;
    const { newText, newCursorPos } = applyCompletion(this.elInput.value, cursorPos, item.label);
  ...
  showResult(rows: unknown[], elapsedMs: number): void {
    ...
    const tbody = (rows as Record<string, unknown>[])
      .map((row) => `<tr>${cols.map((col) => `<td>${renderValue(row[col])}</td>`).join('')}</tr>`)
      .join('');
