...
export function detectPropertyType(
  ...
  if (EMAIL_KEY_PATTERN.test(key) || EMAIL_VALUE_PATTERN.test(strVal))
    return "email";
  if (URL_KEY_PATTERN.test(key) || URL_VALUE_PATTERN.test(strVal)) return "url";
  if (DATE_KEY_PATTERN.test(key) || DATE_VALUE_PATTERN.test(strVal))
    return "date";
...
export function validatePropertyValue(
  ...
  switch (type) {
    case "email": {
      ...
      return ok
        ? { valid: true }
        : { valid: false, message: "有効なメールアドレスを入力してください" };
    }
    case "url": {
      try {
        new URL(value);
    ...
    case "number": {
      ...
      return ok
        ? { valid: true }
        : { valid: false, message: "有効な数値を入力してください" };
    }
    default:
      return { valid: true };
  }
