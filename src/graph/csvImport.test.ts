...
import { importCsv, parseCsv } from './csvImport.js';
...
describe('importCsv', () => {
  ...
  it('エッジ列からエッジを自動生成する', () => {
    ...
    const result = importCsv(db, csv, {
      nodeLabel: 'Person',
      edgeColumns: [{ column: 'manager', edgeType: 'REPORTS_TO', direction: 'out' }],
    });
  ...
  it('direction: in でエッジの向きが逆になる', () => {
    ...
    importCsv(db, csv, {
      nodeLabel: 'Person',
      edgeColumns: [{ column: 'subordinate', edgeType: 'MANAGES', direction: 'in' }],
    });
    const edges = db.getAllEdges();
  ...
  it('無効なノードラベルでエラーをスローする', () => {
    const csv = 'name\nAlice';
    expect(() => importCsv(db, csv, { nodeLabel: '123invalid', edgeColumns: [] })).toThrow();
  });
  ...
  it('無効なエッジタイプでエラーをスローする', () => {
    ...
    expect(() =>
      importCsv(db, csv, {
        nodeLabel: 'Person',
        edgeColumns: [{ column: 'rel', edgeType: 'invalid-type', direction: 'out' }],
      }),
    ).toThrow();
  });
  ...
  it('ヘッダー行のみのCSVでエラーをスローする', () => {
    const csv = 'name,age';
    expect(() => importCsv(db, csv, { nodeLabel: 'Person', edgeColumns: [] })).toThrow();
  });
});
