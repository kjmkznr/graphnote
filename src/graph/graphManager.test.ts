...
function makeStorage(initial: Record<string, string> = {}): IAsyncStorage & {
  ...
  return {
    ...
    listGraphMeta: vi.fn(async () =>
      [...metaStore.values()].sort((a, b) => a.createdAt - b.createdAt),
    ),
    putGraphMeta: vi.fn(async (meta: GraphMeta) => {
      metaStore.set(meta.id, meta);
    }),
    deleteGraphMeta: vi.fn(async (id: string) => {
      metaStore.delete(id);
    }),
  };
}
...
describe("GraphManager", () => {
  ...
  it("createGraph adds a new graph", async () => {
    ...
    await manager.init();
    vi.stubGlobal("crypto", {
      randomUUID: vi.fn().mockReturnValue("new-graph-id"),
    });
  ...
  it("deleteGraph removes a graph", async () => {
    ...
    await manager.init();
    vi.stubGlobal("crypto", {
      randomUUID: vi.fn().mockReturnValue("second-id"),
    });
  ...
  it("deleteGraph throws when only one graph remains", async () => {
    const manager = new GraphManager(storage as never);
    await manager.init();
    await expect(manager.deleteGraph("test-uuid-1234")).rejects.toThrow(
      "最後のグラフは削除できません",
    );
  });
  ...
  it("renameGraph throws when graph not found", async () => {
    ...
    await manager.init();
    await expect(
      manager.renameGraph("nonexistent", "New Name"),
    ).rejects.toThrow("グラフが見つかりません");
  });
