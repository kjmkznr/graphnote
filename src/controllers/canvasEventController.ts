...
function buildNodeTooltipContent(node: RawNode): string {
  ...
  if (label)
    lines.push(`<span class="tooltip-label">:${escHtml(label)}</span>`);
...
function buildEdgeTooltipContent(edge: RawEdge): string {
  ...
  const propEntries = Object.entries(edge._properties).filter(
    ([k]) => !skipKeys.has(k),
  );
  if (propEntries.length > 0) {
    lines.push('<div class="tooltip-props">');
...
function showTooltip(
  ...
  requestAnimationFrame(() => {
    ...
    if (rect.right > window.innerWidth)
      tooltipEl.style.left = `${x - rect.width - 8}px`;
    if (rect.bottom > window.innerHeight)
      tooltipEl.style.top = `${y - rect.height - 8}px`;
  });
}
