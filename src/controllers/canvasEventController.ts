...
export class CanvasEventController {
  ...
  handleCanvasEvent(event: CanvasEvent): void {
    switch (event.kind) {
      ...
      case 'edge-drag-cancelled': return this.ctx.canvas.setMode('edit');
      ...
      case 'bg-tap':           return this.ctx.canvas.clearHighlight();
  ...
  private handleDeleteSelected(nodeGnIds: GnId[], edgeGnIds: GnId[]): void {
    ...
    for (const gnId of edgeGnIds) {
      try { this.ctx.db.deleteEdge(gnId); } catch { /* ignore */ }
    }
    for (const gnId of nodeGnIds) {
      try { this.ctx.db.deleteNode(gnId); } catch { /* ignore */ }
    }
