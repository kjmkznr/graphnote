...
export function setupModeControls(ctx: ToolbarContext): void {
  ...
  document.addEventListener("keydown", (e) => {
    if (e.key === "Escape" && ctx.canvas.getMode() === "node")
      applyMode("edit");
  });
...
export function setupToolbarButtons(ctx: ToolbarContext): void {
  ...
  byId(DOM_IDS.layoutCoseBtn)?.addEventListener("click", () =>
    ctx.canvas.applyLayout("cose"),
  );
  byId(DOM_IDS.layoutCircleBtn)?.addEventListener("click", () =>
    ctx.canvas.applyLayout("circle"),
  );
  byId(DOM_IDS.layoutConcentricBtn)?.addEventListener("click", () =>
    ctx.canvas.applyLayout("concentric"),
  );
  byId(DOM_IDS.layoutGridBtn)?.addEventListener("click", () =>
    ctx.canvas.applyLayout("grid"),
  );
  ...
  byId(DOM_IDS.resetBtn)?.addEventListener("click", () => {
    ...
    ctx.captureForUndo();
    ctx.db.reset();
    clearSaved().catch((err) =>
      console.warn("Failed to clear saved graph:", err),
    );
    ...
    buildShareUrl(ctx.db, ctx.canvas.getPositions(), ctx.canvas.getViewport())
      .then((url) => {
        ...
        if (url.length > 100_000) {
          showToast(
            "グラフが大きすぎるためURLでの共有ができません。JSON エクスポートをご利用ください。",
            "warn",
          );
          return;
        }
  ...
  byId(DOM_IDS.importCsvBtn)?.addEventListener("click", () => {
    showCsvImportDialog(ctx.registry).then((result) => {
      ...
      try {
        ctx.captureForUndo();
        const { nodeCount, edgeCount, skippedEdges } = importCsv(
          ctx.db,
          result.csvText,
          result.options,
        );
        ...
        ctx.refreshAndSave();
        const msg =
          `CSVインポート完了: ノード ${nodeCount} 件、エッジ ${edgeCount} 件` +
