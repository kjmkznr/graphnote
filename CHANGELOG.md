...
#### 開発環境
- `nix develop` で `node_modules/.bin` が PATH に含まれるよう `flake.nix` に `shellHook` を追加
- `flake.nix` のパッケージ宣言を簡素化
- direnv 用の `.envrc` を追加
- `.gitignore` に `.direnv` を追加

#### メンテナンス
- 不要なスタブ型定義パッケージ `@types/marked`・`@types/cytoscape` を削除（各ライブラリが自前の型定義を提供しているため）
...
#### 新機能
- サイドバーでノードプロパティ更新後にキャンバスを再描画するよう改善
- ツールバーにキーボード操作およびクリック対応のドロップダウンメニューを追加

#### バグ修正
- ドロップダウンメニューがヘッダーの下にはみ出せない問題を修正

#### リファクタリング
- `dataset` プロパティアクセスの簡素化とドロップダウン処理ロジックの改善

#### メンテナンス
- `flake.lock` を最新の nixpkgs リビジョンに更新
...
#### 新機能
- ラジアルおよび階層レイアウトとUI切り替えコントロールを追加
- グラフレンダリング、ノード配置、DBキャッシュのスケーラビリティを改善
- 保存・レンダリング・グラフ操作にデバウンス・スロットリング・キャッシュを導入して最適化
- ダッシュボードUIをセクションコンテナで再構成しスタイリングを改善
- スクラップブックUIのレイアウト・フィルタリング・インタラクションを刷新
- デザイントークンを Cytoscape スタイルに統合し CSS を整理

#### リファクタリング
- ESLint/Prettier を Biome に置き換え、フォーマットを統一
- 非 null アサーションを排除し、UI・コントローラーモジュール全体のエラーハンドリングを改善
- DOM 要素 ID 定数の集約
- コールバック登録パターンの統一
- 無音 catch を warn ログ化
- マジック値の命名化
- `persistence.ts` のリストア処理重複を解消
- `db.ts` の Edge 作成 Cypher 重複を解消
- `nodeTypeStyleDialog` / `edgeTypeStyleDialog` の `renderList` を共通化
- `main.ts` のインラインエラー HTML を外出し
- タイプスタイルダイアログの行スタイルとダイアログコンテナのレイアウトを改善
